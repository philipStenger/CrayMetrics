...
Date Created: 25/07/2023 - Directt copy of redcrayfish module as of this date
Last Edit: 25/07/2023, Corwin Broekhuizen
Description: A module to define the packhorse crayfish class and all related functions to this class.
...
SOFT = 1
EGGS = 1
LEGAL_SIZE = 216 #The minimum legal tail length of a packhorse lobster in mm


"""Define crayfish class"""
class PackhorseCray(object):
    """
    PackhorseCray class defines an object associated with a single crayfish, it will be used to store infomation about that crayfish. 
    Attributes:
        .weight
        .length
        .area
    ...
    def __init__(self, *, id=None, catch_id=None, weight=None, length=None, area=None, time_caught=None, \
    ...
    def __str__(self):
        ...
        return f'Object: PackhorseCray, ID: {self.id}, Catch ID: {self.catch_id}'
    
    def __repr__(self):
        """Returns a string of a Cray object in the following form:"""
        return f'PackhorseCray(id={self.id}, catch_id={self.catch_id}, weight={self.weight}, length={self.length}, area={self.area}, \
            time_caught={self.time_caught}, location_caught={self.location_caught}, sex={self.sex}, is_legal={self.is_legal}, \
    ...
    def checkLegality (self):
        ...
        if (self.sex == MALE):
            self.is_legal = ((self.length >= LEGAL_SIZE) and (self.is_soft == SOFT))

        elif (self.sex == FEMALE):
            self.is_legal = ((self.length >= LEGAL_SIZE) and (self.is_soft != SOFT) and (self.has_eggs != EGGS))
